...
export default function MyCart({ navigation }) {
    ...
    const confirmOrder = async () => {
        try {
            ...
            setToken(Parsed.token)
            if (Parsed !== null) {
                await axios.post(`https://multivendor-apex.herokuapp.com/api/servicecart/confirm-service`, {
                }, {
                    headers: {
                        Authorization: Parsed.token
                    }
    ...
    const deleteItem = async (id) => {
        try {
            ...
            const Parsed = JSON.parse(Json);
            setToken(Parsed)
            await axios.delete(`https://multivendor-apex.herokuapp.com/api/servicecart/delete-service`,
                {
                    headers: {
                        "Authorization": Parsed.token
                    },
    ...
    const getData = async () => {
        ...
        try {
            ...
            setToken(Parsed.token)
            if (Parsed !== null) {
                await axios.get(`https://multivendor-apex.herokuapp.com/api/servicecart/`, {
                    headers: {
                        Authorization: Parsed.token
                    }
                }).then(res => {
            ...
            <View style={{ flexShrink: 0, marginBottom: 80 }}>
                <ScrollView>
                    <View style={{ marginTop: 40, marginLeft: 20, marginRight: 20 }}>
                        <Text style={{ fontWeight: '700', color: '#535365', fontSize: 20 }}>Service To</Text>
                    </View>
                        ...
                        data ? data.services.map(item =>
                            <View style={{ flexDirection: 'row', backgroundColor: 'white', padding: 20, marginTop: 20, borderRadius: 10, marginLeft: 20, marginRight: 20 }}>
                                <View style={{ flexDirection: 'column', justifyContent: 'space-around', alignItems: 'center' }}>
                                    <Image style={{ height: 100, width: 90, borderRadius: 10, marginBottom: 5 }} source={{ uri: item.images }} />
                                ...
                                <Text><Image style={{ height: 10, width: 10 }} source={require('../../../assets/clock.png')} /> Expiry Date 20 jul 2022</Text>
...

                                    {
                                        item.discountedprice ?
                                            ...
                                            : 
                                            <View style={{ flexDirection: 'row', marginTop: 5 }}>
                                                 <Text style={{ marginRight: 20, color: 'black', fontSize: 20, fontWeight: '700' }}>₹ {item.price}</Text>

                                            </View>
                                    }
                                    <Text style={{ color: '#00C897' }}>1 offer applied</Text>
                                    <Text style={{ color: 'black' }}>Delivery by 11AM, Tomorrow</Text>
                                    <TouchableOpacity onPress={() => deleteItem(item.serviceId)} style={{ alignItems: 'flex-end' }}>
